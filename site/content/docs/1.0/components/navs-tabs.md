...
layout: docs
title: Navs and tabs
description: Documentation and examples for how to use ndb-uikit's included navigation components.
group: components
aliases: "/docs/1.0/components/navs/"
toc: true
---

## Base nav

Navigation available in ndb-uikit share general markup and styles, from the base `.nav` class to the active and disabled states. Swap modifier classes to switch between each style.
...
As part of ndb-uikit's evolving CSS variables approach, navs now use local CSS variables on `.nav`, `.nav-tabs`, and `.nav-pills` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
Use the tab JavaScript plugin—include it individually or through the compiled `ndb-uikit.js` file—to extend our navigational tabs and pills to create tabbable panes of local content.
...
triggerTabList.forEach(triggerEl => {
  const tabTrigger = new NdbUikit.Tab(triggerEl)
...
ndb-uikit.Tab.getInstance(triggerEl).show() // Select tab by name

const triggerFirstTabEl = document.querySelector('#myTab li:first-child button')
ndb-uikit.Tab.getInstance(triggerFirstTabEl).show() // Select first tab
```
...
<script>
  ...
  const firstTab = new NdbUikit.Tab(firstTabEl)
...
const tab = new NdbUikit.Tab(someTabTriggerEl)
...
```js
const tab = ndb-uikit.Tab.getInstance('#trigger') // Returns a ndb-uikit tab instance
```
...
```js
const tab = ndb-uikit.Tab.getOrCreateInstance('#trigger') // Returns a ndb-uikit tab instance
```
