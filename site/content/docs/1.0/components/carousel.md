...
```js
const carousel = new NdbUikit.Carousel('#myCarousel')
```
...
const carousel = new NdbUikit.Carousel(myCarouselElement, {
  interval: 2000,
  wrap: false
})
```
...
| `getInstance` | Static method which allows you to get the carousel instance associated to a DOM element, you can use it like this: `ndb-uikit.Carousel.getInstance(element)` |
| `getOrCreateInstance` | Static method which returns a carousel instance associated to a DOM element or create a new one in case it wasn't initialized. You can use it like this: `ndb-uikit.Carousel.getOrCreateInstance(element)` |
...
ndb-uikit's carousel class exposes two events for hooking into carousel functionality. Both events have the following additional properties:
