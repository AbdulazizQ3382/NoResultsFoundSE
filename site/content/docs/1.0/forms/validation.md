...
Here's how form validation works with ndb-uikit:

- HTML form validation is applied via CSS's two pseudo-classes, `:invalid` and `:valid`. It applies to `<input>`, `<select>`, and `<textarea>` elements.
- ndb-uikit scopes the `:invalid` and `:valid` styles to parent `.was-validated` class, usually applied to the `<form>`. Otherwise, any required field without a value shows up as invalid on page load. This way, you may choose when to activate them (typically after form submission is attempted).
...
To fix [issues with border radius](https://10.0.224.36:8080/twbs/ndb-uikit/issues/25110), input groups require an additional `.has-validation` class.
